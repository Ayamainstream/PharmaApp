...
import com.example.pharmacy.Activities.OrderActivity;
import com.example.pharmacy.Models.Drugs;
...
public class OrderRecyclerViewAdapter extends RecyclerView.Adapter<OrderRecyclerViewAdapter.FoodViewHolder>{
    ...
    List<Drugs> cloneDrugsList = new ArrayList<>();

    Context context;
    private DrugRecyclerViewAdapter.OnItemClickListener listenerInc;
    private DrugRecyclerViewAdapter.OnItemClickListener listenerDec;
    ...
    public void onBindViewHolder(@NonNull OrderRecyclerViewAdapter.FoodViewHolder holder, int i) {
        ...
        holder.total.setText(String.valueOf(currentFood.getCount() * currentFood.getPrice()));
     ...
     */
    void checkDiff() {
        if (cloneDrugsList.size() == foodList.size()) {
            for (int i = 0; i < cloneDrugsList.size(); i++) {
                if (cloneDrugsList.get(i).getCount() != foodList.get(i).getCount()) {
                    notifyItemChanged(i);
                }
            }
        } else {
     ...
     */
    public void setOnItemClickListenerInc(DrugRecyclerViewAdapter.OnItemClickListener listener) {
     ...
     * to listen for click event
     */
    public void setOnItemClickListenerDec(DrugRecyclerViewAdapter.OnItemClickListener listener) {
