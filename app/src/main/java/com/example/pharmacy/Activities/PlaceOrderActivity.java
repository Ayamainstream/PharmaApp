...
import com.example.pharmacy.Fragments.BottomSheetDialog;
import com.example.pharmacy.Models.FirebaseOrder;
import com.example.pharmacy.R;
import com.example.pharmacy.RoomDatabase.FoodViewModel;
import com.example.pharmacy.Utilises.InternetConnection;
...
public class PlaceOrderActivity extends AppCompatActivity implements BottomSheetDialog.SheetListener {
    ...
    public void orderNow(View view) {
        ...
        if (!address.trim().isEmpty()) {
            ...
            Toast.makeText(this, "Пожалуйста, введите свой адрес", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void onButtonClicked() {
        if (InternetConnection.checkConnection(this)) {
            addOrderToFirebase();
            Toast.makeText(this, "Ваш заказ размещен", Toast.LENGTH_SHORT).show();
            foodViewModel.resetAllOrders();
            finish();
        } else {
            Toast.makeText(this, "Вам нужен интернет для завершения заказа!", Toast.LENGTH_LONG).show();
        }
    }
