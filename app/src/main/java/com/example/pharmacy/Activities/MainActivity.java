...
import com.example.pharmacy.BuildConfig;
import com.example.pharmacy.Fragments.HomeFragment;
import com.example.pharmacy.Fragments.MyOrdersFragment;
import com.example.pharmacy.Fragments.TermsFragment;
...
public class MainActivity extends AppCompatActivity
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_home) {
            ...
            try {
                ...
                shareIntent.putExtra(Intent.EXTRA_SUBJECT, "Pharmacy");
                String shareMessage = "\nЗаказать необходимые лекарства из приложения Аптека\n";
    ...
    private void updateLogoutLogout(FirebaseUser currentUser) {
        ...
        if (currentUser == null) {
            ...
            nameNumberHeader.setText("Войдите, чтобы иметь возможность заказывать!");
        } else {
        ...
        if (requestCode == RC_SIGN_IN) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                Snackbar.make(getWindow().getDecorView().getRootView()
                        , "Не удалось войти, повторите попытку!", Snackbar.LENGTH_LONG)
